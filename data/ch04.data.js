...
const CH04 = {
  meta: { id:"ch04", week:4, title:"리스트", nextTeaser:"트리", nextHint:'교수님이 칠판에 <b>가계도</b>랑 <b>토너먼트 대진표</b>를 그리셨대요… 자료구조 수업 맞죠?' },
  ...
  cpl: {
    "study-A":"4주차 · 월 — 배열의 한계 자습","trial-A":"4주차 · 월 — 이사 비용 시련","il-A":"4주차 · 월요일 밤",
    "study-B":"4주차 · 화 — 연결 리스트 자습","trial-B":"4주차 · 화 — 삽입 시련","il-B":"4주차 · 화요일 밤",
    "tutor":"4주차 · 수 — 과외",
    "study-C":"4주차 · 목 — 삭제와 순회 자습","trial-C":"4주차 · 목 — 삭제 시련","il-C":"4주차 · 목요일 밤",
    "study-D":"4주차 · 금 — 이중 연결 자습","trial-D":"4주차 · 금 — 이중 연결 시련",
  ...
  study: {
    A: { day:"월요일", label:"유닛 A", title:"배열의 한계와 연결 표현", doneLabel:"시련 — 이사 비용 계산 ▶", beats:[
      ...
      {gate:{id:"c4-g-ptr", q:'링크는 <b>포인터</b>다 — 1주차의 <span class="mono">&</span> 와 <span class="mono">*</span>, 확실히 기억하나?', basics:[
      ...
      {gate:{id:"c4-g-mem", q:'힙이라는 <b>영역</b> 이야기가 나왔다 — 프로그램이 쓰는 메모리 공간이 <b>몇 개의 구역으로 나뉘는지</b>, 들어 본 적 있나?', basics:[
        ...
        {who:"book", say:'"이름 조심 — 여기서의 <b>스택 영역</b>은 함수 호출이 쌓이는 메모리 구역의 이름이다. 3주차에 만든 자료구조 스택과 <b>쌓이는 원리(LIFO)는 같지만 별개</b>다. 함수가 호출되면 그 함수의 지역 변수들이 스택 영역에 쌓였다가, 함수가 return하면 통째로 걷힌다."'}
      ]}},
    ...
    B: { day:"화요일", label:"유닛 B", title:"단순 연결 리스트 — 만들기와 삽입", doneLabel:"시련 — 연결 리스트 삽입 ▶", beats:[
      ...
      {who:"book", say:'"노드를 C로 선언하자 — 두 줄을 천천히 읽는 게 이번 장의 첫 고비다. 1행: list_node 구조체를 가리키는 포인터 타입에 <b>list_pointer</b>라는 별칭을 붙인다(1주차의 typedef). 2행: 노드는 <b>데이터 필드</b>(data)와, <b>자기와 같은 타입의 노드를 가리키는 링크 필드</b>(link)로 이루어진다 — 2주차에 이름만 익혀 둔 <b>자체 참조 구조</b>가 드디어 본업을 시작한다. 다음 노드도 같은 모양이니, 링크의 타입은 자기 자신일 수밖에."',
  ...
  interludes: {
    A: [
      ...
      {who:"나", face:"me-proud", text:'(답장) 그게 정확히 오늘 배운 <b>배열의 삽입</b>이다. 좌석이 붙어 있으니 중간에 끼려면 뒤가 전부 밀리는 거야. 내일은 안 밀리는 방법을 배운다 — 각자 흩어져 앉는 대신, 다음 사람이 어느 자리에 있는지 쪽지를 들고 있는 방식.'}
    ],
    B: [
      {who:"도윤", face:"doyun", text:'(문자) 쌤, 보물찾기 해 보셨어요? 쪽지 찾으면 그 쪽지에 다음 장소가 적혀 있는 거요. 오늘 동아리에서 했는데 마지막 쪽지가 없어져서 다 망했어요.'},
      {who:"나", face:"me", text:'(답장) 그거 오늘 배운 자료구조랑 똑같은 구조다. 쪽지가 끊기면 뒤가 전부 미아 — 그래서 쪽지를 고쳐 쓰는 순서까지 정해져 있어. 내일 과외 때 그 얘기 하자.'}
    ],
    C: [
      {who:"도윤", face:"doyun", text:'(문자) 쌤, 아까 유튜브 뒤로가기를 눌렀는데요, 문득 얘는 제가 어디서 왔는지 어떻게 아는 걸까 싶어서요. 앞으로 가기도 되잖아요.'},
      {who:"나", face:"me-proud", text:'(답장) 좋은 질문이다. 한 방향 쪽지로는 "왔던 곳"을 모른다 — 내일 배울 게 정확히 그거다. 쪽지를 양손에 한 장씩 들면 된다.'}
    ]
  },
