"use strict";
/* 챕터 6 데이터 — "트리 순회" = 4장(B) (강의 5장 중반 18~28매 + 신규: 레벨 순회·노드 수/높이·수식 트리 — 천인국 8.5~8.9 재서술 · 규약 v1.4)
   순회 3종(재귀) → 반복 중위(스택)·레벨 순회(큐) → 구조 재귀(count/height/copy/equal) → 수식 트리(3장 계산기 회수).
   서사: 협박 문자 2(책을 안다) + 단서 ② + 도발장 2. */
const CH06 = {
  meta: { id:"ch06", week:4, part:"B", title:"트리 순회", sub:"전위·중위·후위", nextTeaser:"히프와 이진 탐색 트리",
  ...
  cpl: {
    "study-A":"4장(B) · 월 — 순회 3종 자습","trial-A":"4장(B) · 월 — 순회 시련","il-A":"4장(B) · 월요일 밤",
    "study-B":"4장(B) · 화 — 스택·큐 순회 자습","trial-B":"4장(B) · 화 — 레벨 순회 시련","il-B":"4장(B) · 화요일 밤",
    "tutor":"4장(B) · 수 — 과외",
    "study-C":"4장(B) · 목 — 구조 재귀 자습","trial-C":"4장(B) · 목 — 구조 재귀 시련","il-C":"4장(B) · 목요일 밤",
    "study-D":"4장(B) · 금 — 수식 트리 자습","trial-D":"4장(B) · 금 — 수식 트리 시련",
    "saturday":"4장(B) · 토 — 보충/A+","sunday":"4장(B) · 월 — 쪽지시험"
  },
  ...
  tutorPassMsg:'printf 자리 하나로 전위·중위·후위가 갈린다는 거, 오늘 확실히 잡았어요. 스택이면 깊이 먼저, 큐면 층 먼저 — 2장 배운 게 여기서 다시 나올 줄은 몰랐어요.',
  ...
  study: {
    A: { day:"월요일", label:"유닛 A", title:"순회 — 세 가지 방문 순서", doneLabel:"시련 — 순회 3종 ▶", beats:[
      ...
      {gate:{id:"c6-g-rec2", q:'코드로 가기 전에 — 순회 함수는 <b>자기 자신을 다시 호출한다</b>(재귀 호출). 이런 함수가 끝없이 반복되지 않고 <b>반드시 끝나는</b> 이유, 설명할 수 있나?', basics:[
        {who:"book", say:'"용어부터 분명히 — <b>재귀 호출(recursive call)</b>이란 함수가 실행 도중 <b>자기 자신을 다시 호출</b>하는 것이다. inorder 안의 inorder(ptr->left_child)가 그것이다. 중요한 사실: 두 호출은 이름만 같을 뿐 <b>별개의 실행</b>이다 — 각 호출은 자기만의 ptr 값을 가지며, 호출될 때마다 그 정보가 3장에서 본 메모리의 <b>스택 영역</b>에 한 층씩 쌓인다."'},
    ...
    B: { day:"화요일", label:"유닛 B", title:"반복 순회 — 스택과 큐의 재등장", doneLabel:"시련 — 레벨 순회 ▶", beats:[
      ...
      {who:"book", say:'"재귀가 해 주던 일의 정체는 <b>\'되돌아갈 조상 기억하기\'</b>다(호출이 스택 영역에 쌓였다 걷히는 것). 그렇다면 — 그 기억을 우리가 직접 하면 재귀 없이도 된다. 도구는 이미 있다: <b>2장(A)의 스택</b>(push/pop). 왼쪽으로 내려가며 만나는 조상을 전부 쌓아 두고, 막히면 꺼내며(pop) 방문한다."',
      ...
      {who:"book", say:'"이번엔 전혀 다른 방문 순서가 필요하다 — <b>층별로</b>, 위에서 아래로, 같은 층에서는 왼쪽부터 방문하고 싶다. 어제의 세 순회는 전부 한쪽 끝까지 먼저 내려가는 <b>깊이 우선</b>이라 층별 방문이 안 된다. 필요한 것은 \'<b>발견한 순서 그대로</b> 처리하기\' — 2장(A)의 <b>큐</b>(addq/deleteq)다. 이것이 이번 주에 새로 배우는 <b>레벨 순회(level-order)</b>다."',
      ...
      {say:'같은 트리인데 스택을 쓰면 깊이 우선, 큐를 쓰면 층 순서 — 방문 순서는 노드를 <b>어느 자료구조에 보관하느냐</b>가 정하고 있었다. 2장에서 만든 스택과 큐가 이렇게 다시 쓰인다.', mood:"proud"}
    ]},
    ...
    D: { day:"금요일", label:"유닛 D", title:"수식 트리 — 그 계산기의 정체", doneLabel:"시련 — 수식 트리 ▶", beats:[
      {say:'금요일. 오늘 책의 첫 줄을 읽고 잠시 멈췄다 — <b>"2장(B)의 계산기를 기억하는가."</b> …후위 표기 계산기. 그걸 여기서 왜.'},
      ...
      {who:"book", say:'"이제 놀라운 사실 — 이 트리를 세 가지 순회로 방문하며 출력해 보라. <b>중위 순회의 출력은 3 × 4 + 5</b>(우리가 쓰는 중위 표기), <b>후위 순회의 출력은 3 4 × 5 +</b> — 2장(B) 계산기에 넣던 바로 그 후위 표기다. <b>전위 순회의 출력은 + × 3 4 5</b>(전위 표기라 부른다). 세 가지 표기법은 별개의 발명품이 아니라 <b>한 트리를 세 가지 순서로 순회한 결과</b>였다."',
       table:'<table class="trip"><tr><th>중위 순회</th><td class="mono">3 × 4 + 5</td><td>사람의 표기</td></tr><tr><th>후위 순회</th><td class="mono">3 4 × 5 +</td><td>2장(B) 계산기의 표기</td></tr><tr><th>전위 순회</th><td class="mono">+ × 3 4 5</td><td>전위 표기</td></tr></table>'},
      ...
      {check:{id:"c6D-3", stem:'2장(B)의 <b>후위 표기 계산기</b>(스택)와 수식 트리의 관계로 옳은 것은?',
  ...
  hints: {
    ...
    D:["【모양】 피연산자 = 리프, 연산자 = 내부 노드. 먼저 계산할 것일수록 아래 — 괄호는 모양에 녹아 있다.",
       "【표기법】 중위 순회의 출력 = 중위 표기, 후위 순회의 출력 = 후위 표기(2장(B) 계산기), 전위 순회의 출력 = 전위 표기.",
  ...
  interludes: {
    A: [
      ...
      {who:"나", face:"me-proud", text:'(답장) 둘 다 맞다 — 그게 오늘 배운 순회다. 조상 먼저가 전위, 자손 먼저가 후위. 틀린 순서가 있는 게 아니라 <b>용도가 다른</b> 것뿐이야. 소개는 전위가, 감사 인사는 후위가 어울리겠네.'}
    ],
    B: [
      {who:"도윤", face:"doyun", text:'(문자) 쌤, 엄마 심부름으로 마트 갔는데요. 계산대는 온 순서대로 계산해 주잖아요. 근데 물건 찾을 땐 한 통로를 끝까지 훑고, 없으면 되돌아 나와서 다음 통로로 갔거든요. 오늘 배운 거랑 비슷하지 않아요?'},
      {who:"나", face:"me", text:'(답장) 제대로 봤다 — 계산대 줄이 큐(먼저 온 순서대로), 통로 훑기가 스택(한쪽 끝까지 갔다가 되돌아오기) 방식이다. 오늘 배운 레벨 순회와 중위 순회가 정확히 그 두 가지야.'}
    ],
    C: [
      {who:"도윤", face:"doyun", text:'(문자) 쌤, 내일 배울 게 <b>수식 트리</b>라는데요, 교수님이 "계산기를 만들어 본 사람은 내일 소름 돋을 것"이래요. 저 소름 돋을 준비 됐어요. 옛날에 쌤이 계산기 가르쳐 주셨잖아요!'},
      {who:"나", face:"me-proud", text:'(답장) …설마 그때 그 후위 표기가. 내일 같이 소름 돋자.'},
  ...
  tutorQs: [
    ...
    {id:"Q3", boss:true, ask:'마지막이요. 재귀를 흉내 낼 땐 <b>스택</b>이고 층별로 돌 땐 <b>큐</b>잖아요 — 노드를 저장하는 <b>자료구조만</b> 바꿨는데, 왜 하나는 <b>깊이</b>로 파고들고 하나는 <b>층</b>으로 도는 거예요?',
     choices:[
      {text:'"꺼내는 순서가 반대라서야. 스택은 <b>방금 넣은 것부터</b>(LIFO)라 한쪽 끝까지 파고들게 되고, 큐는 <b>먼저 넣은 것부터</b>(FIFO)라 발견된 층 순서를 지키지."', correct:true, fb:'꺼내는 규칙이 곧 방문 순서네요. 2장에서 배울 땐 이런 데 쓰일 줄 몰랐어요.'},
  ...
  pool: [
    ...
    {id:"P12", unit:"D", stem:'수식 트리에 <b>괄호 노드가 없는</b> 이유는?',
     ...
     choices:[
      ...
      {text:"컴퓨터는 괄호 문자를 처리할 수 없어서",correct:false,mc:"char-myth",fb:"2장(B)에서 괄호 검사까지 했다 — 못 다루는 게 아니라 필요가 없다."},
