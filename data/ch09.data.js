"use strict";
/* 챕터 9 데이터 — "그래프 탐색" = 5장(B) (강의 6장 23~37매 · 제작 규약 v1.4 · 주간 루프 공용 러너 사용)
   DFS 절차(미로 회수) → DFS 코드(재귀) → BFS(큐·"미로를 큐로" 정식 회수) → 응용(연결 요소·신장 트리).
   서사: 문자 4(순수 압박) + 여백 메모 발견(주인공 자력 — 단서 등록 없음, ⑤는 5장(C)) + 도발장 5(오래된 문투의 문제 — 궁지 심리).
...
const CH09 = {
  meta: { id:"ch09", week:5, part:"B", title:"그래프 탐색", sub:"빠짐없이 도는 두 가지 방법", nextTeaser:"가중치 그래프",
  ...
  intro: [
    ...
    {who:"나", face:"me-think", text:'<span class="inner">미로라. …2장(B)의 그 금요일 밤이 떠오른다. 갈림길을 기억해 두고, 막히면 되돌아가던 — 그때 내 손에 있던 것이 스택이었다. 미로의 칸을 정점으로, 통로를 간선으로 바꿔 보면… 그렇군. 같은 문제다.</span>'},
  ...
  cpl: {
    "study-A":"5장(B) · 월 — DFS 절차 자습","trial-A":"5장(B) · 월 — DFS 시련","il-A":"5장(B) · 월요일 밤",
    "study-B":"5장(B) · 화 — DFS 코드 자습","trial-B":"5장(B) · 화 — 코드 시련","il-B":"5장(B) · 화요일 밤",
    "tutor":"5장(B) · 수 — 과외",
    "study-C":"5장(B) · 목 — BFS 자습","trial-C":"5장(B) · 목 — BFS 시련","il-C":"5장(B) · 목요일 밤",
    "study-D":"5장(B) · 금 — 탐색 응용 자습","trial-D":"5장(B) · 금 — 응용 시련",
    "saturday":"5장(B) · 토 — 보충/A+","sunday":"5장(B) · 월 — 쪽지시험"
  },
  ...
  tutorPassMsg:'막다른 길에서 정확히 갈림길로 돌아오는 게 스택 덕분이라는 거, 오늘 완전히 잡았어요. 아 맞다 — 내일 학교에서는 <b>너비 우선</b>이라는 것도 한대요. 이름부터 반대 같아요.',

  /* ================= 자습 ================= */
  study: {
    A: { day:"월요일", label:"유닛 A", title:"깊이 우선 탐색(DFS) — 절차", doneLabel:"시련 — DFS 추적기 ▶", beats:[
      {say:'책을 펴기 전에, 2장(B)의 금요일 밤을 떠올린다 — 미로. 갈림길을 스택에 쌓고, 막히면 꺼내서 되돌아가던 그 밤.', mood:"awkward"},
      ...
      {who:"book", say:'"두 번째 도구는 2장(B)의 그 <b>스택</b>이다. 깊이 우선의 걸음걸이: 지금 선 정점의 인접 중 <b>방문 안 한 하나</b>를 골라 나아간다 — 한 길을 끝까지 파고든다. 그럼 고르지 <b>않은</b> 길들은? 버리는 게 아니다. 지나온 정점을 스택에 쌓아 두었다가, 인접이 전부 방문된 <b>막다른 곳</b>에 이르면 스택에서 꺼내 직전 갈림길로 되돌아간다 — <b>백트래킹</b>. 되돌아간 곳에 아직 안 가 본 길이 남았으면 거기서 다시 파고든다."'},
    ...
    B: { day:"화요일", label:"유닛 B", title:"깊이 우선 탐색 — 코드", doneLabel:"시련 — DFS 코드 판독기 ▶", beats:[
      ...
      {who:"book", say:'"dfs를 코드로. 4장(B)의 순회와 <b>같은 뼈대의 재귀</b>다 — 자신을 방문하고, 이웃마다 같은 일을 반복시킨다."',
      ...
      {who:"book", say:'"한 줄씩. ① <b>visited[v] = TRUE</b> — 들어서자마자 발자국부터. 이 줄이 for보다 앞이어야 한다: 뒤로 미루면 이웃의 재귀가 아직 표시 없는 v로 <b>되돌아 들어와 끝나지 않는다</b>. ③ for는 v의 인접 리스트를 처음부터 끝까지 — 리스트가 오름차순이므로 \'작은 번호부터\' 규약이 저절로 지켜진다. ⑤ 재귀 호출 — 어제의 스택은 어디 갔나? <b>함수 호출 자체가 시스템 스택에 쌓인다</b>(4장(B)에서 본 그대로). 되돌아가기(백트래킹)는 재귀가 <b>리턴하는 것</b>이다."'},
      {check:{id:"c9B-1", stem:'이 코드에서 어제의 <b>스택</b>은 어디에 있는가?',
        ...
        choices:[
          ...
          {text:"이 코드에는 스택에 해당하는 것이 없다",correct:false,mc:"no-stack",fb:"보이지 않을 뿐 있다 — 4장(B)의 재귀 순회와 같은 원리다."}]}},
    ...
    C: { day:"목요일", label:"유닛 C", title:"너비 우선 탐색(BFS)", doneLabel:"시련 — BFS 추적기 ▶", beats:[
      ...
      {who:"book", say:'"<b>너비 우선 탐색(BFS, breadth first search)</b> — 한 길을 파고드는 대신, <b>출발점에서 가까운 정점부터 골고루</b> 넓혀 간다. 도구는 스택이 아니라 <b>큐</b>(2장(A))다. 절차: ① 출발 정점을 방문하고 큐에 넣는다. ② 큐에서 정점을 꺼내, 그 인접 중 <b>미방문 정점을 모두</b> — 방문 표시를 하며 — 큐에 넣는다. ③ 큐가 빌 때까지 반복."'},
      {who:"book", say:'"코드. 큐는 3장의 노드로 만든 <b>동적 연결 큐</b> — 크기 제한이 없다."',
      ...
      {who:"book", say:'"이제 미뤄 둔 질문에 답하자 — 2장(B)의 미로에서 「가 볼 곳」 저장소를 <b>스택 대신 큐</b>로 바꾸면? 스택은 한 길을 끝까지 파고들었다(깊이 우선). 큐는 <b>출발 주변의 가까운 칸부터 골고루</b> 넓혀 간다 — 지금 배운 너비 우선이다. 같은 미로, 같은 코드에서 저장소 하나로 탐색의 성격이 갈린다. 그리고 하나 더 — 트리에서 BFS를 돌리면 4장(B)의 <b>레벨 순회</b> 그대로다: 레벨 순회가 큐를 썼던 이유가 이것이다."'},
      ...
      {check:{id:"c9C-3", stem:'트리에서 BFS를 실행하면 4장(B)의 어떤 순회와 같은가?',
    ...
    D: { day:"금요일", label:"유닛 D", title:"탐색의 응용 — 연결 요소와 신장 트리", doneLabel:"시련 — 연결·신장 판독기 ▶", beats:[
      ...
      {who:"book", say:'"셋째 — <b>신장 트리(spanning tree)</b>. 연결 그래프를 탐색하면 모든 정점을 방문한다. 이때 <b>실제로 지나간 간선들만</b> 남겨 보라 — 모든 정점을 포함하면서 사이클이 없는 트리, 즉 신장 트리가 된다(dfs가 만들면 깊이 우선 신장 트리, bfs가 만들면 너비 우선 신장 트리). 간선 수는 4장에서 본 그대로 <b>정점 수 − 1</b>."',
       viz:{type:"graph", nodes:[{id:0,x:90,y:28},{id:1,x:45,y:95},{id:2,x:135,y:95},{id:3,x:14,y:168},{id:4,x:64,y:168},{id:5,x:116,y:168},{id:6,x:166,y:168},{id:7,x:90,y:240}],
  ...
  hints: {
    A:["【절차】 방문 표시(발자국) → 미방문 인접 중 작은 번호로 전진 → 갈림길은 스택 → 막히면 pop 백트래킹 → 스택 비면 종료.",
       ...
       "【미로】 2장(B) 미로의 전략과 같다 — 칸=정점, 통로=간선."],
    B:["【코드 뼈대】 visited[v]=TRUE(먼저!) → printf → for(인접 리스트) → if(!visited) → dfs 재귀.",
       "【스택의 행방】 재귀 호출 = 시스템 스택. 백트래킹 = 리턴. (4장(B) 회수)",
  ...
  pool: [
    ...
    {id:"P03", unit:"A", stem:'2장(B)의 <b>미로</b>를 그래프로 보면, 옳은 대응은?',
    ...
    {id:"P05", unit:"B", stem:'dfs 코드의 <span class="mono">for (w = graph[v]; w; w = w->link)</span> 가 하는 일은?', mono:true,
     okfb:'정점 v의 인접 리스트를 처음부터 끝(NULL)까지 걷는다 — 5장(A)의 그 리스트다.',
     choices:[
    ...
    {id:"P12", unit:"D", stem:'정점 n개, 간선 e개의 연결 그래프에서 <b>신장 트리</b>가 갖는 간선 수는?', mono:true,
     okfb:'모든 정점을 잇는 트리 — 4장의 성질 그대로 n−1개다.',
     choices:[
  ...
  interludes: {
    ...
    C: [
      {who:"도윤", face:"doyun", text:'(문자) 쌤, 오늘 수업에서 교수님이 「탐색이 지나간 간선만 남기면 트리가 된다」고 했는데요. 아무렇게나 생긴 그래프에서 트리가 나온다는 게 마법 같아요. 내일 그거 배우는 거 맞죠?'},
      {who:"나", face:"me-proud", text:'(답장) 맞다, 내일이 그 이야기다. 마법이 아니라 — 탐색은 각 정점에 "처음 도착한 길" 하나씩만 쓰거든. 정점마다 들어온 길이 하나면, 그게 바로 트리다. 내일 밤 내가 먼저 정리해 두마.'},
  ...
  aplusSkin: {
    ...
    offer:[
      {who:"발신 번호 없음", face:"📵", text:'(문자) 지난주는 운이 좋았던 걸로 해 두지. …이번 것은 다르다. 탐색 셋 — 못 풀면, 말했던 대로 답안지가 어머님 앞으로 간다.'},
