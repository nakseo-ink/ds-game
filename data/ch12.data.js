"use strict";
/* 챕터 12 데이터 — "고급 정렬" = 6장(B) (강의 7장 6~19매 + 종합 비교 보강(분할검토 v1.0) · 제작 규약 v1.5 · 주간 루프 공용 러너)
   퀵(원본) → 합병(원본) → 히프(원본 — 4장(C) 이월 회수) → 종합 비교(보강).
...
const CH12 = {
  meta: { id:"ch12", week:6, part:"B", title:"고급 정렬", sub:"분할 정복의 셋과 총정리", nextTeaser:"해시",
  ...
  cpl: {
    "study-A":"6장(B) · 월 — 퀵 정렬 자습","trial-A":"6장(B) · 월 — 퀵 시련","il-A":"6장(B) · 월요일 밤",
    "study-B":"6장(B) · 화 — 합병 정렬 자습","trial-B":"6장(B) · 화 — 합병 시련","il-B":"6장(B) · 화요일 밤",
    "tutor":"6장(B) · 수 — 과외",
    "study-C":"6장(B) · 목 — 히프 정렬 자습","trial-C":"6장(B) · 목 — 히프 시련","il-C":"6장(B) · 목요일 밤",
    "study-D":"6장(B) · 금 — 종합 비교 자습","trial-D":"6장(B) · 금 — 종합 시련","il-D":"6장(B) · 금요일 밤",
    "saturday":"6장(B) · 토 — 보충/A+","sunday":"6장(B) · 월 — 쪽지시험"
  },
  ...
  study: {
    ...
    C: { day:"목요일", label:"유닛 C", title:"히프 정렬 — 트리를 빌려 온 정렬", doneLabel:"시련 — 히프 추적기 ▶", beats:[
      {say:'세 번째. 낯익은 얼굴이다 — 4장에서 만든 그 히프. "정렬에 다시 쓴다"던 예고를 회수할 시간이다.', mood:"proud"},
      {who:"book", say:'"기억을 꺼내자 — <b>max 히프</b>(4장(C)): 완전 이진 트리이면서 <b>모든 부모 ≥ 자식</b>. 배열의 <b>1번 칸부터</b> 담으면 i의 자식은 2i와 2i+1, 부모는 i/2였다. 그리고 최댓값은 언제나 루트(1번 칸)에 있다. <b>히프 정렬(heap sort)</b>은 이 성질을 정렬에 빌려 온다: ① 배열 전체를 <b>최대 히프로 구성</b>한다 ② <b>루트(최댓값)를 하나씩 꺼내</b> 배열 뒤에서부터 채운다. 큰 것부터 뒤로 쌓이니, 다 꺼내면 앞에서부터 오름차순이다."'},
      ...
      {who:"book", say:'"도구는 하나면 된다 — <b>adjust</b>: 루트만 어긋났을 수 있는 트리를 받아, 루트 값을 <b>내려보내</b> 히프로 고치는 함수다. 4장(C) delete_max에서 마지막 원소를 루트로 올린 뒤 하던 그 일 — 두 자식 중 <b>큰 쪽</b>과 비교하고, 자식이 크면 자식을 끌어올리며 한 층 내려간다."',
  ...
  tutorQs: [
    ...
    {id:"Q2", ask:'합병 정렬은 왜 <b>추가 배열</b>이 필요해요? 두 리스트가 어차피 한 배열 안에 나란히 있는데, 그냥 그 자리에서 섞으면 되잖아요.',
     choices:[
      {text:'"제자리에서 섞으려면 작은 값을 앞으로 <b>끼워 넣어야</b> 하는데, 그 순간 뒤의 원소들이 전부 한 칸씩 밀려 — 1장의 배열 삽입, 그 비용이 매번 터지는 거야. 그러면 회전당 O(n)이던 이동이 무너져서 합병의 자랑인 O(n log n) 보장이 사라져. 결과를 옆 배열에 차곡차곡 담는 건 그 밀기를 통째로 사는 값인 셈이지."', correct:true, fb:'아, 끼워 넣으면 밀리니까… 추가 배열이 밀기 비용을 대신 내주는 거네요.'},
